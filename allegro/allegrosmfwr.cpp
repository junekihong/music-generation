...
void Alg_smf_write::write_update(Alg_update_ptr update)
{
	...
	// After both events are seen, we write the data. (See below.)
	    ...
	    if (!strcmp(Alg_atom_name(update->parameter.a), "majora"))
		    keysig_mode = 'M';
		else keysig_mode = 'm';
		keysig_when = update->time;
	}
...
void Alg_smf_write::write_track(int i)
{
	...
	if (i == 0) { // track 0 may have tempo and timesig info
		if (seq->map.last_tempo_flag || seq->map.beats.len > 0) {
			pending = push(pending, new event_queue('c', 0.0, 0, NULL));
		}
		if (seq->time_sig.len > 0) {
	...
	while (pending) {
		...
		if (current->type == 'n') {
			...
			current->index++; // -R
			if (current->index < seq->map.beats.len) {
				current->time = seq->map.beats[current->index].beat;
 				pending = push(pending, current);
			} else {
				delete current;
			}
			...
			current->index++;
			if (current->index < seq->time_sig.len) {
