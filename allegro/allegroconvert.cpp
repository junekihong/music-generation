			 ...
			 bool flatten_flag)
{
	...
	seq->map.beats.len = 1; // remove contents of tempo map
	seq->map.last_tempo = tempo / 60.0; // set the new fixed tempo
	 // (allegro uses beats/second so divide bpm by 60)
	seq->map.last_tempo_flag = true;
}
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < seq->tracks.len; i++) {
		events += seq->tracks[i]->len;
	}
	printf("%d tracks, %d events\n", seq->tracks.len, events);
	printf("wrote %s\n", outfilename);
